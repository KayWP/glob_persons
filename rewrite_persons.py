...
class Person:
    ...
    def split_values(
        ...
        if not hasattr(attr_list[0], field_name):
            raise AttributeError(
                f"{field_name} is not a valid field for {attribute_name}"
            )
        ...
        for a in attr_list:
            ...
            for sep in separators:
                split_parts = [
                    part.strip() for val in split_parts for part in val.split(sep)
                ]

            # Filter out unused_remains
            split_parts = [
                part for part in split_parts if part and part not in unused_remains
            ]
    ...
    def link_values(
        ...
        attribute_name: str,
        field_name: str,
        log_file: str = "unmatched_values.txt",
    ):
        """
...
class PersonList:
    ...
    def split_list_valuessplit_values(
        self,
        attribute_name: str,
        field_name: str,
        separators: List[str],
        ...
        for p in self.persons:
            p.link_valuessplit_values(
                self, attribute_name, field_name, separators, unused_remains, exceptions
            )

    def link_list_values(
        self,
        mapping: dict,
        attribute_name: str,
        field_name: str,
    ...
    def to_csv(
        ...
        makeIdentities=True,
        makeStatuses=True,
        makeLocation_relations=True,
        makeRelations=True,
        makeEvents=True,
        makeExternalReferences=True,
    ):
        """
        ...
        if makeAppellations:
            ...
            appellationsFrame = pd.DataFrame(
                appellationsFrame,
                columns=[
                    "URI",
                    "Observation",
                    ...
                    "Toponym",
                    "Observation source",
                    "Location in Observation Source",
                    "Reconstruction Source",
                    "Location in Reconstruction Source",
                    "Comment",
                ],
            )
                ...
                for identity in p.identities:
                    identitiesFrame.append(
                        [
                            ...
                            self._format_value(
                                identity.location_in_reconstruction_source
                            ),
                            self._format_value(identity.comment),
                        ]
                    )
            identitiesFrame = pd.DataFrame(
                identitiesFrame,
                columns=[
                    "URI",
                    "Observation",
                ...
                for status in p.statuses:
                    statusesFrame.append(
                        [
                            ...
                            self._format_value(
                                status.location_in_reconstruction_source
                            ),
                            self._format_value(status.comment),
                        ]
                    )
            statusesFrame = pd.DataFrame(
                statusesFrame,
                columns=[
                    "URI",
                    "Observation",
        ...
        if makeLocation_relations:
            ...
            locationRelationFrame = pd.DataFrame(
                locationRelationFrame,
                columns=[
                    "URI",
                    "Observation",
        ...
        if makeExternalReferences:
            ...
            externalReferencesFrame = pd.DataFrame(
                externalReferencesFrame,
                columns=[
                    "URI",
                    "Reconstruction ID",
    ...
    def update_db(
        ...
        if makeAppellations:
            ...
            for p in tqdm(self.persons):
                for a in p.appellations:
                    ...
                    new_appellation_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_appellation_sql.reconstruction_source = a.reconstruction_source
                    new_appellation_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeActive_as:
            ...
            for p in tqdm(self.persons):
                for a in p.active_as:
                    ...
                    new_activeAs_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_activeAs_sql.reconstruction_source = a.reconstruction_source
                    new_activeAs_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeIdentities:
            ...
            for p in tqdm(self.persons):
                for a in p.identities:
                    ...
                    new_Identity_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_Identity_sql.reconstruction_source = a.reconstruction_source
                    new_Identity_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeStatuses:
            ...
            for p in tqdm(self.persons):
                for a in p.statuses:
                    ...
                    new_Status_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_Status_sql.reconstruction_source = a.reconstruction_source
                    new_Status_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeLocation_relations:
            ...
            # Create an empty object to bind to the table
            class locationRelation_sql(object):
                pass
            ...
            for p in tqdm(self.persons):
                for a in p.location_relations:
                    ...
                    new_locationRelation_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_locationRelation_sql.reconstruction_source = (
                        a.reconstruction_source
                    )
                    new_locationRelation_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeRelations:
            ...
            for p in tqdm(self.persons):
                for a in p.relations:
                    ...
                    new_relation_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_relation_sql.reconstruction_source = a.reconstruction_source
                    new_relation_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeEvents:
            ...
            for p in tqdm(self.persons):
                for a in p.events:
                    ...
                    new_event_sql.location_in_observation_source = (
                        a.location_in_observation_source
                    )
                    new_event_sql.reconstruction_source = a.reconstruction_source
                    new_event_sql.location_in_reconstruction_source = (
                        a.location_in_reconstruction_source
                    )
        ...
        if makeExternalReferences:
            ...
            class external_reference_sql(object):
                pass
